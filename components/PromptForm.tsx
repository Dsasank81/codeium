...
const defaultPrompt = `A warm wide establishing shot on a gentle 35mm spherical lens captured with an ARRI Alexa Mini glides slowly across a sunlit workshop filled with scattered tools, glowing monitors, and hanging blueprint paper, the frame textured with a soft Kodak 200T film grain. In the center of the shot sits Codey, a small, charming robot with round LED eyes, brushed aluminum panels, and tiny articulated hands, his screen flickering with symbols as he struggles through lines of messy code; he emits a faint whir as he slumps in mild exhaustion. The scene is set mid-morning, sunlight streaming through tall windows, dust motes floating in golden shafts of light across a cozy, creative workspace decorated with circuit boards, coffee mugs, and neon sticky notes. A group of friendly developers enter the frame — seen only as warm silhouettes moving around Codey — gently rotating his chair, straightening his cables, and opening his code on nearby monitors. The camera shifts into a slow dolly-in, bringing us closer as highlighted snippets of code are reorganized, comments appear like bright annotations, and unnecessary lines fade away. The lighting becomes subtly brighter, lifting into a hopeful, soft-glow high-key setup, emphasizing clarity and renewal. Codey’s eyes light up in brighter blues and greens as he compiles faster, screen refresh humming with energy; tiny LED strips on his chassis flicker excitedly as he learns new tools and reorganizes his workflow. Ambient audio consists of gentle typing, quiet servo sounds, and a warm electronic hum, underscored by soft, uplifting synth chords. The color palette transitions from cluttered warm neutrals to clean teals, whites, and soft yellows, symbolizing clarity and growth. As Codey looks up at the developers, he chirps joyfully, “I feel smarter already!” in a cheerful digital tone, embodying the spirit of augmentation, collaboration, and innovation… shot in cinematic style.`;
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const [prompt, setPrompt] = useState(initialValues?.prompt ?? defaultPrompt);
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      // When resetting the form (e.g., "New Video"), restore defaults
